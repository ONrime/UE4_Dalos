...
void ALoby_PlayerController::AddPlayerInfo_Implementation()
{
	...
	if (gameState != nullptr && gameState->connetedPlayers.Num() > 0) {
		...
		for (int i = 0; i < connectedPlayersInfo.Num(); i++) {
			UE_LOG(LogTemp, Warning, TEXT("playerSettingsddd: %s"), *gameState->connetedPlayers[i].playerReadyStatus);
		}
	}
	//connectedPlayersInfo = gameState->connetedPlayers; // 나중에 인자 지우고 해보기
	/*for (int i = 0; i < connectedPlayersInfo.Num(); i++) {
		UE_LOG(LogTemp, Warning, TEXT("connectedPlayersInfo: %s"), *connectedPlayersInfo[i].playerReadyStatus);
